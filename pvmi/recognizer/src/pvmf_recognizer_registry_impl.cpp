...
PVMFStatus PVMFRecognizerRegistryImpl::CheckForDataAvailability()
{
    ...
    LOGINFO((0, "PVMFRecognizerRegistryImpl::CheckForDataAvailability - IN "));
    ...
    LOGINFO((0, "PVMFRecognizerRegistryImpl::CheckForDataAvailability - OUT - retval = %d",
             retval));
...
PVMFStatus PVMFRecognizerRegistryImpl::RunRecognitionPass(PVMFRecognizerResult& aResult)
{
    ...
    for (it = iPlugInParamsVec.begin(); it != iPlugInParamsVec.end(); ++it)
    {
        ...
        if ((*it)->iInUse == true)
        {
            ...
            else
            {
                ...
                {
                    LOGINFO((0, "PVMFRecognizerRegistryImpl::RunRecognitionPass Recognize plug-in Start i=%d iAdditionalBytesRequired = %d",
                             i, aResult.iAdditionalBytesRequired));
    ...
    LOGINFO((0, "PVMFRecognizerRegistryImpl::RunRecognitionPass OUT "));
...
{
    LOGINFO((0, "PVMFRecognizerRegistryImpl::DoRecognize - IN "));
    if (iRecognizerCurrentCmd.empty() == true)
    {
    ...
    LOGINFO((0, "PVMFRecognizerRegistryImpl::DoRecognize - OUT status = %d",
             status));
